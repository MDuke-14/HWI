              ...
              <DialogContent className="bg-[#1a1a1a] border-gray-700 text-white max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                    ...
                    Histórico de Localização - {locationUser?.full_name || locationUser?.username}
                  </DialogTitle>
                </DialogHeader>

                <div className="mt-4 space-y-4">
                  {/* Date Selector */}
                  <div className="flex items-center gap-4 bg-[#0f0f0f] p-4 rounded-lg">
                    <div className="flex-1">
                      <Label className="text-gray-300 mb-2 block">Data</Label>
                      <Input
                        type="date"
                        value={locationHistoryDate}
                        onChange={(e) => {
                          setLocationHistoryDate(e.target.value);
                          if (locationUser) {
                            fetchUserLocationHistory(locationUser.id, e.target.value, e.target.value);
                          }
                        }}
                    ...
                    <div className="flex gap-2 pt-6">
                      <Button
                        onClick={() => {
                          ...
                          setLocationHistoryDate(dateStr);
                          if (locationUser) fetchUserLocationHistory(locationUser.id, dateStr, dateStr);
                        }}
                        variant="outline"
                        size="sm"
                      ...
                      <Button
                        onClick={() => {
                          ...
                          setLocationHistoryDate(today);
                          if (locationUser) fetchUserLocationHistory(locationUser.id, today, today);
                        }}
                        variant="outline"
                        size="sm"
                        className="border-gray-600 text-gray-300"
                  ...
                  {locationLoading ? (
                    ...
                    <div className="space-y-4">
                      <LocationMap
                        locations={userLocationHistory.map(loc => ({
                          id: loc.id,
                          latitude: loc.latitude,
                          longitude: loc.longitude,
                          userName: `${new Date(loc.timestamp).toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}`,
                          address: loc.address?.locality || loc.address?.formatted || loc.location_description || 'Local não identificado',
                          timestamp: loc.timestamp,
                          type: loc.type,
                          color: loc.outside_residence_zone ? 'orange' : 'green'
                        }))}
                        height="350px"
                        zoom={14}
                      />
                      ...
                      <div className="bg-[#0f0f0f] rounded-lg p-4 max-h-48 overflow-y-auto">
                        <h4 className="text-sm font-semibold text-gray-400 mb-3">Registos ({userLocationHistory.length})</h4>
                        <div className="space-y-2">
                          {userLocationHistory.map((loc, idx) => (
                            <div key={loc.id || idx} className="flex items-center justify-between bg-[#1a1a1a] p-3 rounded-lg">
                              <div className="flex items-center gap-3">
                                <div className={`w-3 h-3 rounded-full ${loc.outside_residence_zone ? 'bg-orange-500' : 'bg-emerald-500'}`} />
                                <div>
                                  ...
                                  <p className="text-gray-400 text-xs">
                                    {loc.address?.locality || loc.address?.formatted || loc.location_description || 'Localização sem endereço'}
                                  </p>
                                </div>
                              </div>
                              {loc.outside_residence_zone && (
                                <span className="text-xs bg-orange-600/20 text-orange-400 px-2 py-1 rounded">Fora de Zona</span>
                              )}
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="text-center py-12 text-gray-400">
                      ...
                      <p>Sem registos de localização para esta data</p>
                      <p className="text-sm mt-2">O utilizador pode não ter iniciado ponto com geolocalização ativa</p>
                    </div>
                  )}
                </div>
              </DialogContent>
