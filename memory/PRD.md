...
- **Card "Relatório de Assistência" separado** (5 Março 2026) - Removido campo "Relatório" dos cards de Intervenções. Criado novo card dedicado antes das Assinaturas com CRUD completo (adicionar/editar/eliminar). Cada entrada pode relacionar equipamentos da OT (principal + adicionais) via checkboxes. Novo modelo `RelatorioAssistencia` e 4 endpoints CRUD no backend.
