...
- **Preencher Dias Vazios (Feb 2026)**: Novo endpoint `POST /api/admin/time-entries/fill-empty-days` (admin only) que aceita `{user_id, date_from, date_to}` e preenche cada dia útil sem qualquer registo com duas entradas: 08:00-12:00 + 13:00-17:00 (8h total). Ignora: fins-de-semana, feriados, dias com `time_entries`, dias com pedidos em `vacation_requests` (férias/folga/cancelamento) e dias com falta em `absences_v2`. Idempotente. UI: botão âmbar "Preencher Dias Vazios" (data-testid `fill-empty-days-btn`) ao lado do "Download PDF" em `/admin/time-entries`, abre diálogo de confirmação (`fill-empty-days-dialog`) que usa o período de faturação seleccionado (26→25). Testado via curl: 10 dias preenchidos num intervalo de 15 dias com 4 FDS + 1 feriado; segunda chamada devolve 0 (idempotente).
